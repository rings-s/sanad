<script>
	...
	const uid       = $derived($page.url.searchParams.get('uid') || '');
	const token     = $derived($page.url.searchParams.get('token') || '');
	const hasParams = $derived(!!uid && !!token);
...
<div>
	...
	<p class="text-sm mb-8 text-stone-500 dark:text-stone-400">{$t('auth.resetSubtitle')}</p>
	...
	{#if done}
		...
		<a href="/login" class="btn-primary text-sm">{$t('auth.login')}</a>
	...
	{:else if !hasParams}
		...
		<a href="/forgot-password" class="btn-ghost text-sm">{$t('auth.forgotTitle')}</a>
	...
	{:else}
		...
		<form onsubmit={handleSubmit} novalidate>
			...
			<button type="submit" class="btn-primary w-full py-3 text-base"
				...
				{/if}
				{loading ? $t('ui.loading') : $t('auth.resetButton')}
			</button>
		</form>
	{/if}
</div>
